...
void main() {
  ...
  p += u_p1 * 9.1 * cos(u_scale * 8.7 * p.yx + (newTime) + vec2(-1.2,3.4));
  p += u_p2 * 4.1 * cos(u_scale * 8.7 * p.yx + newTime + vec2(2.2,3.4));
  ...
  // newST.x = mix(st.x, p, 0.1);
  // newST.y = mix(st.y, p, 0.1);
  vec4 color = texture2D(u_texture, p);


  // float r = -0.687328364064339;
  // // float pct = distance(st,vec2(0.4))/r + distance(st,vec2(u_p1,u_p2)/r);
  // float pct = min(distance(st,vec2(u_p1)),distance(st,vec2(u_p2)));
  // vec2 newST = vec2(0.0,0.0);
